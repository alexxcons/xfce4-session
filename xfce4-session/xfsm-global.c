...
gint
xfsm_launch_desktop_files_for_run_hook (gboolean  start_at_spi,
                                      const char *run_hook_name)
{
  const gchar *try_exec;
  const gchar *type;
  const gchar *run_hook;
  const gchar *exec;
  ...
  for (n = 0; files[n] != NULL; ++n)
    {
      ...
      run_hook = xfce_rc_read_entry (rc, "RunHook", NULL);
      if (G_LIKELY (run_hook != NULL))
        {
          if(g_strcmp0(run_hook, run_hook_name) != 0)
            skip = TRUE;
        }
